...
export default function LoginPage() {
  ...
  const t = {
    ...
    subtitle: isAr ? 'أدخل رقم هاتفك للمتابعة' : 'Enter your phone number to continue',
    phonePlaceholder: isAr ? 'مثال: 791234567' : 'e.g. 791234567',
    phoneLabel: isAr ? 'رقم الهاتف' : 'Phone Number',
    sendOtp: isAr ? 'إرسال رمز التحقق' : 'Send OTP',
    ...
    verify: isAr ? 'تحقق والدخول' : 'Verify & Sign In',
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', top: '-10%', right: '-5%', width: 400, height: 400, borderRadius: '50%', background: `radial-gradient(circle, ${DS.colors.gold}22 0%, transparent 70%)` }} />
        <div style={{ position: 'absolute', bottom: '-10%', left: '-5%', width: 350, height: 350, borderRadius: '50%', background: `radial-gradient(circle, ${DS.colors.navyMid}44 0%, transparent 70%)` }} />
      </div>
      ...
      <div style={{
        ...
        <div style={{ padding: '40px 40px 36px' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: DS.colors.navy, margin: 0, letterSpacing: '-0.5px' }}>{t.title}</h1>
            ...
            <form onSubmit={handleSendOtp}>
              ...
              <button
                ...
                style={{
                  ...
                  background: phone.length < 7 || loading
                    ? DS.colors.mutedLight
                    : `linear-gradient(135deg, ${DS.colors.gold}, #e8b04a)`,
                  color: phone.length < 7 || loading ? DS.colors.muted : DS.colors.navy,
                  fontWeight: 700, fontSize: 15, border: 'none', cursor: phone.length < 7 || loading ? 'not-allowed' : 'pointer',
                  transition: 'all 0.2s', boxShadow: phone.length >= 7 && !loading ? `0 4px 20px ${DS.colors.gold}44` : 'none',
                  letterSpacing: '0.3px',
                }}
              >
                {loading ? '...' : t.sendOtp}
          ...
          <div style={{ textAlign: 'center', marginTop: 28, paddingTop: 24, borderTop: `1px solid ${DS.colors.border}`, fontSize: 13, color: DS.colors.muted }}>
            {t.noAccount}{' '}
            <Link href={`/${locale}/signup`} style={{ color: DS.colors.gold, fontWeight: 700, textDecoration: 'none' }}>
              {t.signUp}
            </Link>
          </div>
        </div>
